...
class SaleOrder(models.Model):
    ...
    def _get_invoiced(self):
        ...
        for rec in self:
            invoice_ids_sub = self.env['account.move'].search([('ref', '=',rec.name)])
                        ...
                        if line.product_id.activate_subscription:
                            ...
                            if line.product_id.subscription_plan_id.trial_period:
                                ...
                                if line.product_id.subscription_plan_id.trial_duration_unit == 'week':
                                    date = date + timedelta(weeks=line.product_id.subscription_plan_id.trial_duration)
                            vals = {'customer_name': order.partner_id.id, 'customer_billing_address' : order.partner_invoice_id.id, 'source': 'so', 'so_origin': order.id, 'product_id': line.product_id.id, 'quantity': line.product_uom_qty, 'start_date': date,'tax_id': [
    ...
    def create_invoices(self):
        ...
        for subsc in sale_orders.subscription_ids:
            ...
            if subsc.num_billing_cycle>0:
                start_date = datetime(year=subsc.start_date.year, month=subsc.start_date.month, day=subsc.start_date.day, minute=0, hour=0, second=0)
                if not isinstance(start_date,datetime):
                    start_date = datetime(*start_date.timetuple()[:6])
                end_date = datetime(year=subsc.end_date.year, month=subsc.end_date.month, day=subsc.end_date.day, minute=0, hour=0, second=0)
