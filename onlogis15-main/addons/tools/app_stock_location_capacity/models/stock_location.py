...
class Location(models.Model):
    ...
    occupied_unit = fields.Float('Qty Occupied', digits='Product Unit of Measure', compute='_compute_occupied', store=False)
    # todo: 预计的
    ...
    occupied_percent = fields.Float('Occupied(%)', compute='_compute_occupied', store=False)

    # 计算该位置的数量，主要是quant
    @api.depends('capacity_type', 'occupied_order', 'capacity_unit')
    def _compute_occupied(self):
        for rec in self:
            ...
            if rec.usage in ('internal', 'transit'):
                ...
                elif rec.capacity_type == 'unit':
                    try:
                        quants = self.env['stock.quant'].sudo().search([('location_id', 'child_of', rec.id)])
                        occupied_unit = sum(quants.mapped('quantity'))
                        occupied_percent = 100.0 * occupied_unit / rec.capacity_unit
                    except:
                        pass
            rec.occupied_unit = occupied_unit
