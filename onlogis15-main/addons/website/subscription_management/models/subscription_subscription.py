...
class subscription_subscription(models.Model):
    ...
    def action_invoice_create(self, grouped=False, final=False):
        ...
        if invoices:
            ...
            if sent_invoice:
                ...
                emails_from = self.env['mail.template']._render_template(template.email_from, 'account.move',
                                                                         invoice.id)
            ...
            start_date = datetime(year=self.start_date.year, month=self.start_date.month, day=self.start_date.day,
                                  minute=0, hour=0,
                                  second=0) if self.source == 'manual' else self.start_date + relativedelta(days=1)
            ...
            if subscription.num_billing_cycle != subscription.invoice_count:
                if self.num_billing_cycle > 0:
                    ...
                    end_date = datetime(year=self.end_date.year, month=self.end_date.month, day=self.end_date.day,
    ...
    def _prepare_invoice_line(self, qty):
        ...
        _logger.critical('-----------333333333----------------  {0}'.format(account))
        res = {
            'name': name,
                ...
                for invoice_id in current_rec.invoice_ids:
                    if invoice_id.amount_residual_signed > 0.0:
                        ...
                        self.env['account.payment'].sudo().create(
                            {'journal_id': invoice_id.journal_id.id, 'amount': invoice_id.amount_total,
                             'payment_date': invoice_id.invoice_date, 'communication': invoice_id.name,
                             'payment_type': 'inbound', 'payment_method_id': self.env['account.payment.method'].search(
                                [('payment_type', '=', 'inbound')], limit=1).id, 'partner_type': 'customer',
                             'partner_id': invoice_id.partner_id.id, }).post()
