...
class AddProduct(models.TransientModel):
    ...
    def create_sale_order_line(self):
        ...
        if sale_order.pricelist_id and sale_order.partner_id:
            ...
            if not override:
                line_id.price_unit = self.env['account.tax']._fix_tax_included_price_company(line_id._get_display_price(product), product.
                taxes_id, line_id.tax_id, line_id.company_id)
            else:
                line_id.price_unit = self.env['account.tax']._fix_tax_included_price_company(self.product_id.lst_price, product.
                taxes_id, line_id.tax_id, line_id.company_id)
