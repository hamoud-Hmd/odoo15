...
class PalletAutoBarcode(models.Model):
    _inherit = 'stock.pallet'

    @api.model
    def create(self, vals_list):
        res = super(PalletAutoBarcode, self).create(vals_list)
        ean = generate_ean(str(res.id))
        res.update({'barcode': ean})
        return res

class PackAutoBarcode(models.Model):
    _inherit = 'stock.quant.package'

    @api.model
    def create(self, vals_list):
        res = super(PackAutoBarcode, self).create(vals_list)
        ean = generate_ean(str(res.id))
        res.update({'barcode': ean})
        return res
